...
function update(text) {
    ...
    //Update code
    //result_element.innerHTML = text.replaceAll(new RegExp(">", "g"), ">").replaceAll(new RegExp("<", "g"), "<"); /* Global RegExp */
...
function executeCode() {
    //console.log(document.querySelector("#editing").innerHTML);
    //console.log(document.getElementById("highlighting-content").innerTe);
    //console.log(document.getElementById("nom_exo").innerText.split(' ')[0]);
    $.ajax({
        url: "./compiler.php",
        method: "POST",
        data: {
    ...
    function getContent() {
        var d = document.getElementsByClassName("output")[0];
        var text = d.innerHTML;
        console.log("test");
        if (text === "Exercice fini :)") {
            alert("test");
        }
    }
