...
public class TestEntierMax {
    ...
    public static void testIndiceMaxTableau(Tableaux tab) throws Exception {
        try { // Si la classe javatests.Tableaux ne comporte pas de méthode entierMax
            Class.forName("javatests.Tableaux").getDeclaredMethod("entierMax");
        } catch (Exception e) {
            // Message d'erreur
            throw new Exception("La méthode entierMax est mal orthographiée");
        }
        // On regarde si la méthode entierMax est publique
        if (!(Class.forName("javatests.Tableaux").getDeclaredMethod("entierMax").getModifiers() == Modifier.PUBLIC)) {
            // Message d'erreur
            throw new Exception("La méthode entierMax n\'est pas publique");
        }
        // On regarde si la méthode entierMax renvoie bien un entier
        if (!(Class.forName("javatests.Tableaux").getDeclaredMethod("entierMax").getReturnType().toString().equals("int"))) {
        ...
        if (!(entierMaxTest == entierMaxStatic)) {
            // Message d'erreur
            throw new Exception("La méthode entierMax ne fonctionne pas. \n Expected : " + entierMaxStatic);
        }
    }
