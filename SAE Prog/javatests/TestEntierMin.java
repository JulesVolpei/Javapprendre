...
public class TestEntierMin {
    ...
    public static void testIndiceMinTableau(Tableaux tab) throws Exception {
        ...
        if (!(Class.forName("javatests.Tableaux").getDeclaredMethod("entierMin").getModifiers() == Modifier.PUBLIC)) {
            // Message d'erreur
            throw new Exception("La méthode entierMin n\'est pas publique");
        }
        // On regarde si la méthode entierMin retourne bien un entier
        if (!(Class.forName("javatests.Tableaux").getDeclaredMethod("entierMin").getReturnType().toString().equals("int"))) {
            // Message d'erreur
