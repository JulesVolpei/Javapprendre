...
public class TestAfficherLeMotEnMinuscule {
    ...
    public static void testAffichageMotEnMinuscule() throws Exception {
        try {
            Class.forName("javatests.ManipulationString").getMethod("afficherLeMotEnMinuscule", String.class);
        } catch (Exception e) {
            throw new Exception("Il ne faut pas modifier la méthode afficherLeMotEnMinuscule(String mot)");
        }
        if (!(Class.forName("javatests.ManipulationString").getMethod("afficherLeMotEnMinuscule", String.class).getModifiers() == Modifier.PUBLIC)) {
            throw new Exception("Il faut que la méthode afficherLeMotEnMinuscule soit public");
        }
    ...
    public static void testAffichageMotEnMinuscule(List<String> liste) throws Exception {
        try {
            Class.forName("javatests.ManipulationString").getMethod("afficherLeMotEnMinuscule", String.class);
        } catch (Exception e) {
            throw new Exception("Il ne faut pas modifier la méthode afficherLeMotEnMinuscule(String mot)");
        ...
        if (!liste.get(7).equals(GenererMot.getMot().toLowerCase())) {
            throw new Exception("Le mot n\'est pas affiché en minuscule. \n Attendu : " + GenererMot.getMot().toLowerCase() + "\n Reçu : " + liste.get(7));
        }
    }
